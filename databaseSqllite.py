...
class RefereeDb():
    ...
    def getAllMentoringSessions(self) -> dict:
        # figure out if it is the fall or spring season.  Get reports for just that
        # range.

        retVal = {}
        #sql = f"select r.lastname, r.firstname, ms.position, ms.date from mentor_sessions ms join referees r \
        #      on ms.mentee = r.id where ms.date between '{range[0]}' and '{range[1]}'"
        ...
        rows = r.fetchall()
        for row in rows:
            retVal[f'{row[1]} {row[0]}'] = [ row[2], row[3], row[4], row[5], row[6] ]
        return retVal
